...
TEST_CASE("Insert number") {
    ...
    auto sineWithNumber = operationReplacer.change(formulaSine);
    ...
    auto divisionWithNumber = operationReplacer.change(formulaDivision);
    CHECK(divisionWithNumber.toString() == "((x/2)*1)");

    auto divisionWithTwoNumbers = operationReplacer.change(divisionWithNumber);
